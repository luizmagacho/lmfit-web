...
export default function BillingClient() {
  ...
  const isFree = !data?.stripeSubscriptionStatus || data?.plan === "free";
  const needsPayment = ["past_due", "unpaid", "canceled"].includes(data?.stripeSubscriptionStatus || "");
  ...
  return (
    <div className="max-w-5xl space-y-8 pb-12">
      ...
      {!isFree ? (
        <div className="rounded-2xl border p-6 md:p-8 bg-[var(--card-bg)] shadow-sm" style={{ borderColor: lmfitTokens.border }}>
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold ${
                    data?.stripeSubscriptionStatus === "active" ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400" :
                    "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400"
                  }`}>
                    <div className={`w-1.5 h-1.5 rounded-full ${data?.stripeSubscriptionStatus === "active" ? "bg-emerald-500 animate-pulse" : "bg-amber-500"}`} />
                    {data?.stripeSubscriptionStatus === "active" ? "Ativo" : data?.stripeSubscriptionStatus}
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="rounded-3xl border bg-[var(--card-bg)] p-8 flex flex-col shadow-sm relative overflow-hidden" style={{ borderColor: lmfitTokens.border }}>
              ...
              <button
                disabled={actionLoading}
                onClick={() => handleCheckout(isAnnual ? "price_1ThVle2L6koEu6W28ejtHUEs" : "price_1ThVkP2L6koEu6W2c5axNLhh")}
              ...
              <ul className="mt-8 space-y-4 flex-1">
                ...
                <FeatureItem text="PDV Integrado" />
                <FeatureItem text="Gestão Financeira Básica" />
            ...
            <div className="rounded-3xl border-2 border-violet-500 bg-[var(--card-bg)] p-8 flex flex-col shadow-md relative overflow-hidden transform md:-translate-y-2">
              ...
              <button
                disabled={actionLoading}
                onClick={() => handleCheckout(isAnnual ? "price_1ThVle2L6koEu6W27xkRZ8d1" : "price_1ThVkQ2L6koEu6W2H6stMQf3")}
              ...
              <ul className="mt-8 space-y-4 flex-1">
                <FeatureItem text="Até 50 usuários" />
                <FeatureItem text="Emissão de NF-e e NFC-e" />
                <FeatureItem text="Integração InfinitePay" />
                <FeatureItem text="WhatsApp CRM (1 Número)" />
            ...
            <div className="rounded-3xl border bg-[var(--card-bg)] p-8 flex flex-col shadow-sm relative overflow-hidden" style={{ borderColor: lmfitTokens.border }}>
              ...
              <button
                disabled={actionLoading}
                onClick={() => handleCheckout(isAnnual ? "price_1ThVlf2L6koEu6W2IS23cjKw" : "price_1ThVkQ2L6koEu6W2JUJIgZuB")}
              ...
              <ul className="mt-8 space-y-4 flex-1">
                ...
                <FeatureItem text="API Aberta (Integrações)" />
                <FeatureItem text="WhatsApp CRM Ilimitado" />
                <FeatureItem text="Gerente de Conta Dedicado" />
                <FeatureItem text="Treinamento Online" />
              </ul>
            </div>
          </div>
        </div>
